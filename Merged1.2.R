...
EndDate <- "2014-10-01"     ### MUST be at least 1 year later!
...
W <- data.frame(date=as.POSIXct(index(Z)), tides=Z$Tides,lagoon=Z$Lagoon,
                waves=Z$Waves,discharge=Z$Discharge,level=Z$Eta)
...
if(Plotly){
     ...
     C1 <- ggplotly(C[[1]],height=2000)
     C2 <- ggplotly(C[[2]])
     C3 <- ggplotly(C[[3]])
     C4 <- ggplotly(C[[4]] + scale_x_date(date_breaks = "months" , date_labels = "%b"))
     C5 <- ggplotly(C[[5]] + scale_x_date(date_breaks = "months" , date_labels = "%b"))
     C6 <- ggplotly(C[[6]] + scale_x_date(date_breaks = "months" , date_labels = "%b"))
     C7 <- ggplotly(C[[7]] + scale_x_date(date_breaks = "months" , date_labels = "%b"))
     C8 <- ggplotly(H1)
     C9 <- ggplotly(H2)
