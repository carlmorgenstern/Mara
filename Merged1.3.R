...
EndDate <- "2015-10-01"     ### MUST be at least 1 year later!
...
Waves <- read.csv("~/Mara/csv/Waves_longTerm.csv")
h1WY <- read.csv("~/Mara/csv/h1WY00_20.csv")
Tides$time <-as.POSIXct(Tides$time, format = " %d-%b-%Y %H:%M:%S", tz="UTC")  
Lagoon$time <- as.POSIXct(Lagoon$time, format = " %H:%M:%S %m/%d/%Y" , tz="UTC") 
h1WY$time <-   as.POSIXct(h1WY$Date, format = " %H:%M:%S %m/%d/%Y" , tz="UTC") 
Waves$time <-  as.POSIXct(Waves$Date, format = "%d-%b-%Y %H:%M:%S" , tz="UTC")

colnames(h1WY)[1] <- "Discharge"
h1WY$Date <- NULL
Waves$Date <- NULL
...
Waves <- xts(Waves$Wave_Height,Waves$time) 
Discharge <- xts(h1WY$Mean,h1WY$time)

Tides <- Tides[Interval]
Lagoon <- Lagoon[Interval]
Waves <- Waves[Interval]
Discharge <- Discharge[Interval]

z <- merge(Tides,Lagoon,Waves,Discharge)
head(z)
z$Tides < na.approx(z$Tides,rule=2)
z$Lagoon <- na.approx(z$Lagoon,rule=2)
z$Waves <- na.approx(z$Waves,rule=2)
z$Discharge <- na.approx(z$Discharge,rule=2)
...
W <- data.frame(date=as.POSIXct(index(Z)), tides=Z$Tides,lagoon=Z$Lagoon,
                waves=Z$Waves,discharge=Z$Discharge)
...
plot_period_column = function (Data, column) {
        ...
        stat_peaks(color="red",span=30,geom="text",
                   ignore_threshold=0.90, show.legend=TRUE,
...
g <- list()
     ...
     g[[1]] <- ggplot(W,aes(x=date,y=Waves)) + 
               geom_line() +
               ggtitle(paste0("Waves -- Water Year ",FirstWaterYear))+
               theme(plot.title=element_text(size=10)) +
               scale_x_date(date_breaks = "3 months" , date_labels = "%b")

     
     g[[2]] <- ggplot(W,aes(x=date,y=Discharge)) + 
               geom_line() +
               ggtitle(paste0("Discharge -- Water Year ",FirstWaterYear))+
               theme(plot.title=element_text(size=10)) +
               scale_x_date(date_breaks = "3 months" , date_labels = "%b")

     g[[3]] <- ggplot(W,aes(x=date,y=Lagoon)) + 
               geom_line() +
               ggtitle(paste0("Lagoon -- Water Year ",FirstWaterYear))+
               theme(plot.title=element_text(size=10)) +
               scale_x_date(date_breaks = "3 months" , date_labels = "%b")


 wrap_plots(lapply(colnames(W)[c(4,5,3)], plot_data_column, Data = W),ncol=1)
...
for(i in BodiesOfWater){
  for(k in FirstWaterYear:(LastWaterYear-1)){
    C <- list()
    C[[1]] <- g[[1]]  # Waves time series
    C[[2]] <- g[[2]]  # Discharge tine series
    C[[3]] <- g[[3]] # Lagoon time series
    D <- data.frame()
    ...
    for(j in 1:length(Periods)){                                  # create vector of indices to plot over time
      ...
      C[[4]] <- ggplot(P24,aes(x=date,y=totalPower24)) + geom_line() + 
                 ylab("total power") + 
...
if(Plotly){
     ...
     #C1 <- ggplotly(C[[1]])
     C1 <- ggplotly(C[[1]] + scale_x_date(labels = date_format("%b-%y"),date_breaks="3 months"))
     C2 <- ggplotly(C[[2]] + scale_x_date(labels = date_format("%b-%y"),date_breaks="3 months"))
     C3 <- ggplotly(C[[3]] + scale_x_date(labels = date_format("%b-%y"),date_breaks="3 months"))
     C4 <- ggplotly(C[[4]] + scale_x_date(labels = date_format("%b-%y"),date_breaks="3 months"))
     C5 <- ggplotly(C[[5]] + scale_x_date(labels = date_format("%b-%y"),date_breaks="3 months"))
     C6 <- ggplotly(C[[6]] + scale_x_date(labels = date_format("%b-%y"),date_breaks="3 months"))
     C7 <- ggplotly(C[[7]] + scale_x_date(labels = date_format("%b-%y"),date_breaks="3 months"))
     C8 <- ggplotly(H1)
     C9 <- ggplotly(H2)
...
CPlotly <- subplot(C1,C2,C3,C4,C5,C6,C7,C8,C9,nrows = 9,shareX = FALSE ) %>% 
           layout(autosize = FALSE,title = list(text = paste0("Water Years ",
                         FirstWaterYear," - ",LastWaterYear),font = t1,x=10)) %>%
...
CPlotly <- CPlotly %>% layout(annotations = annotations) 
#options(warn = -1)
...
}     # commented out wavelet x BodiesOfWater
