...
ui <- fluidPage(
     plotOutput("timeseriesrow"),
     plotOutput("timeseriescol"),
     plotOutput("freqrow"),
     plotOutput("freqcol"),
     plotOutput("periodrow"),
     plotOutput("periodcol"),
     plotOutput("TidesWavelet"),
     plotOutput("LagoonWavelet"),
     plotOutput("EtaWavelet"),
     plotOutput("ReconstructedTides"),
     plotOutput("ReconstructedTidesError"),
     plotOutput("ReconstructedLagoon"),
     plotOutput("ReconstructedLagoonError"),
     plotOutput("ReconstructedEta"),
     plotOutput("ReconstructedEtaError")
)
...
plot_data_column = function (Data, column) {
    ggplot(Data, aes_string(x = W[,1],y=column)) +
        ...
        ggtitle(paste(WaterYear, column)) +
            theme(plot.title = element_text(size=15),axis.text.x=element_text(size=5.5))+
        xlab("date")
  #        scale_x_date(date_breaks = "months" , date_labels = "%b-%y")
}
output$timeseriesrow <- renderPlot({wrap_plots(lapply(colnames(W)[-1], plot_data_column, Data = W),nrow=1) })
output$timeseriescol <- renderPlot({wrap_plots(lapply(colnames(W)[-1], plot_data_column, Data = W),ncol=1) })


## Spectra ----

for(i in BodiesOfWater){
eval(parse(text=paste0("S",i," <- spec.pgram","(W$",i,
                             ",spans=spectrumWindowSize,log='yes',demean=TRUE,detrend=TRUE,plot=FALSE)")))
     }             
...
output$freqrow <- renderPlot({wrap_plots(lapply(colnames(S)[3:5], plot_power_column,Data=S),nrow=1)})
output$freqcol <- renderPlot({wrap_plots(lapply(colnames(S)[3:5], plot_power_column,Data=S),ncol=1)})

output$periodrow <- renderPlot({wrap_plots(lapply(colnames(S)[3:5], plot_period_column,Data=S),nrow=1) })
output$periodcol <- renderPlot({wrap_plots(lapply(colnames(S)[3:5], plot_period_column,Data=S),ncol=1) })
...
eval(parse(text=paste0("output$",i,"Wavelet <- renderPlot({wt.image(Wavelet",i,
...
eval(parse(text=paste0("RR",i,
                       "<- reconstruct(Wavelet",i,
...
#output$ReconstructedTides <- renderPlot({ggplot(RTides,aes(x=date))+ geom_line(aes(y=error)) })


output$ReconstructedTides <- renderPlot({ggplot(RTides,aes(x=date)) +
...
# 
# output$ReconstructedTidesError <- renderPlot({ggplot(X,aes(x=date,y=error)) + 
#                                   geom_line() +
...
# 
# output$ReconstructedLagoon <- renderPlot({ggplot(ReconstructedLagoon,aes(x=date)) +
#                              geom_line(aes(y=original,color="original"), linewidth=1.0,
#                              linetype="solid")  +
#                              geom_line(aes(y=reconstructed,color="reconstructed"),linewidth=0.4,
#                              linetype="dotted") +
...
# 
# output$ReconstructedLagoonError <- renderPlot({ggplot(X,aes(x=date,y=error)) + 
#                                   geom_line() +
...
ReconstructedEta   <- data.frame(date=RReconstructedEta$series$date,
                                  ...
                                  error = RReconstructedEta$series$Eta-RReconstructedEta$series$Eta.r)

output$ReconstructedEta <- renderPlot({ggplot(ReconstructedEta,aes(x=date)) +
                             geom_line(aes(y=original,color="original"), linewidth=1.0,
                             linetype="solid")  +
                             geom_line(aes(y=reconstructed,color="reconstructed"),linewidth=0.4,
                             linetype="dotted") +
...
output$ReconstructedEtaError <- renderPlot({ggplot(X,aes(x=date,y=error)) + 
                                  geom_line() +
