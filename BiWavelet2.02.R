...
ui <- fluidPage(
     ...
     plotOutput("timeseries4"),
     plotOutput("timeseries5"),
     plotOutput("timeseries6"),
...
output$timeseries1 <- renderPlot({wrap_plots(lapply(colnames(W)[-1], plot_data_column, Data = W),nrow=1) })
output$timeseries2 <- renderPlot({wrap_plots(lapply(colnames(W)[-1], plot_data_column, Data = W),ncol=1) })
...
output$timeseries3 <- renderPlot({wrap_plots(lapply(colnames(S)[3:5], plot_power_column,Data=S),nrow=1)})
output$timeseries4 <- renderPlot({wrap_plots(lapply(colnames(S)[3:5], plot_power_column,Data=S),ncol=1)})
...
output$timeseries5 <- renderPlot({wrap_plots(lapply(colnames(S)[3:5], plot_period_column,Data=S),nrow=1) })
output$timeseries6 <- renderPlot({wrap_plots(lapply(colnames(S)[3:5], plot_period_column,Data=S),ncol=1) })
...
output$TidesWavelet <- renderPlot({wt.image(WaveletTides,
