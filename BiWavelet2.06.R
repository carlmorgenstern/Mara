...
ui <- fluidPage(
     plotlyOutput("timeseriesrow"),
     plotlyOutput("timeseriescol"),
     plotlyOutput("freqrow"),
     plotlyOutput("freqcol"),
     plotlyOutput("periodrow"),
     plotlyOutput("periodcol"),
     plotlyOutput("TidesWavelet"),
     plotlyOutput("LagoonWavelet"),
     plotlyOutput("EtaWavelet"),
...
output$timeseriesrow <- renderPlotly({wrap_plots(lapply(colnames(W)[-1], plot_data_column, Data = W),nrow=1) })
output$timeseriescol <- renderPlotly({wrap_plots(lapply(colnames(W)[-1], plot_data_column, Data = W),ncol=1) })
...
output$freqrow <- renderPlotly({wrap_plots(lapply(colnames(S)[3:5], plot_power_column,Data=S),nrow=1)})
output$freqcol <- renderPlotly({wrap_plots(lapply(colnames(S)[3:5], plot_power_column,Data=S),ncol=1)})

output$periodrow <- renderPlotly({wrap_plots(lapply(colnames(S)[3:5], plot_period_column,Data=S),nrow=1) })
output$periodcol <- renderPlotly({wrap_plots(lapply(colnames(S)[3:5], plot_period_column,Data=S),ncol=1) })
...
eval(parse(text=paste0("output$Reconstructed",i," <- renderPlotly({ggplot(R",i,",aes(x=date)) +
...
eval(parse(text=paste0("X <-R",i,"[trim:(length(R",i,"$date)-trim),]" )))

eval(parse(text=paste0("output$Reconstructed",i,"Error <- renderPlotly({ggplot(X,aes(x=date,y=error)) + 
